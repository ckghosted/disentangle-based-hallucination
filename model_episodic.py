...
class HAL_PN_only(object):
    ...
    def build_model(self):
        ...
        ### episodic validation
        ### dataset
        self.support_features_val = tf.placeholder(tf.float32, shape=[self.n_way_val, self.n_shot_val, self.fc_dim], name='support_features_val')
        self.query_features_val = tf.placeholder(tf.float32, shape=[self.n_query_all_val, self.fc_dim], name='query_features_val')
        self.query_labels_val = tf.placeholder(tf.int32, shape=[self.n_query_all_val], name='query_labels_val')
        ### basic operation
        self.support_feat_flat_val = tf.reshape(self.support_features_val, shape=[-1, self.fc_dim]) ### shape: [self.n_way*self.n_shot, self.fc_dim]
        self.query_labels_vec_val = tf.one_hot(self.query_labels_val, self.n_way_val)
        self.support_class_code_val = self.proto_encoder(self.support_feat_flat_val, bn_train=self.bn_train_hal, with_BN=self.with_BN, reuse=True) #### shape: [self.n_way*self.n_shot, self.fc_dim]
        self.query_class_code_val = self.proto_encoder(self.query_features_val, bn_train=self.bn_train_hal, with_BN=self.with_BN, reuse=True) #### shape: [self.n_query_all, self.fc_dim]
        ### prototypical network data flow using the augmented support set
        self.support_encode_val = tf.reshape(self.support_class_code_val, shape=[self.n_way_val, self.n_shot_val, -1]) ### shape: [self.n_way, self.n_shot, self.fc_dim]
        self.support_prototypes_val = tf.reduce_mean(self.support_encode_val, axis=1) ### shape: [self.n_way, self.fc_dim]
        self.query_tile_val = tf.reshape(tf.tile(self.query_class_code_val, multiples=[1, self.n_way_val]), [self.n_query_all_val, self.n_way_val, -1]) #### shape: [self.n_query_all, self.n_way, self.fc_dim]
        self.logits_pro_val = -tf.norm(self.support_prototypes_val - self.query_tile_val, ord='euclidean', axis=2) ### shape: [self.n_query_all, self.n_way]
        self.loss_pro_val = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.query_labels_vec_val,
                                                                                       logits=self.logits_pro_val,
                                                                                       name='loss_pro_val'))
        self.acc_pro_val = tf.nn.in_top_k(self.logits_pro_val, self.query_labels_val, k=1)
...
#                                                                  var_list=self.trainable_vars)
        ...
        for epoch in range(1, (num_epoch+1)):
            ...
            if not self.val_path is None:
                for ite in tqdm.tqdm(range(1, (n_ite_per_epoch+1))):
                    ##### make episode
                    skip_this_episode = False
                    selected_lbs = np.random.choice(list(self.all_val_labels), self.n_way_val, replace=False)
                    try:
                        selected_indexes = [list(np.random.choice(self.candidate_indexes_each_lb_val[selected_lbs[lb_idx]], self.n_shot_val+self.n_query_all_val//self.n_way_val, replace=False)) \
                                            for lb_idx in range(self.n_way_val)]
                    except:
                        print('[Validation] Skip this episode since there are not enough samples for some label')
                        skip_this_episode = True
                    if skip_this_episode:
                        continue
                    support_features = np.concatenate([self.val_feat_list[selected_indexes[lb_idx][0:self.n_shot_val]] for lb_idx in range(self.n_way_val)])
                    support_features = np.reshape(support_features, (self.n_way_val, self.n_shot_val, self.fc_dim))
                    query_features = np.concatenate([self.val_feat_list[selected_indexes[lb_idx][self.n_shot_val:]] for lb_idx in range(self.n_way_val)])
                    query_labels = np.concatenate([np.repeat(lb_idx, self.n_query_all_val//self.n_way_val) for lb_idx in range(self.n_way_val)])
                    # support_labels = selected_lbs
                    # support_classes = np.array([self.val_class_list_raw[selected_indexes[i][0]] for i in range(self.n_way_val)])
                    loss, acc = self.sess.run([self.loss_pro_val, self.acc_pro_val],
                                              feed_dict={self.support_features_val: support_features,
                                                         self.query_features_val: query_features,
                                                         self.query_labels_val: query_labels,
                                                         # self.support_labels: support_labels,
                                                         # self.support_classes: support_classes,
    ...
    def inference(self,
        ...
        could_load, checkpoint_counter = self.load_hal_pro(hal_from, hal_from_ckpt)
        if True or could_load:
            print(" [*] Load SUCCESS")
            ...
            for ite in tqdm.tqdm(range(1, (n_ite_per_epoch+1))):
                ...
                selected_lbs = np.random.choice(list(self.all_val_labels), self.n_way_val, replace=False)
                # if ite < 5:
                #     print('selected_lbs:', selected_lbs)
                try:
                    selected_indexes = [list(np.random.choice(self.candidate_indexes_each_lb_val[selected_lbs[lb_idx]], self.n_shot_val+self.n_query_all_val//self.n_way_val, replace=False)) \
                                        for lb_idx in range(self.n_way_val)]
                    # if ite < 5:
                ...
                acc_ite_val.append(np.mean(acc))
            print('---- Novel ----')
            print('novel loss: %f, novel accuracy: %f (std: %f)' % \
                (np.mean(loss_ite_val), np.mean(acc_ite_val), np.std(acc_ite_val)))
...
class HAL_PN_GAN(object):
    def __init__(self,
                 ...
                 n_aug=10, ## number of samples per class in the augmented support set
                 n_query_all=75, ## number of samples in the query set
                 n_shot_val=1, ## number of samples per class in the support set of validation/novel episodes
                 n_aug_val=10, ## number of samples per class in the augmented support set of validation/novel episodes
                 fc_dim=512,
                 z_dim=512,
                 z_std=1.0,
                 l2scale=0.001,
        ...
        if not self.val_path is None:
            self.n_way_val = 5
            self.n_shot_val = n_shot_val
            self.n_aug_val = n_aug_val
            self.n_query_all_val = 75
            self.val_base_dict = unpickle(self.val_path)
            self.val_feat_list = self.val_base_dict['features']
            self.val_fname_list = self.val_base_dict['image_names']
    ...
    def build_model(self):
        ...
        self.query_labels_vec_val = tf.one_hot(self.query_labels_val, self.n_way_val)
        ### hallucination flow
        self.hal_feat_val, self.z_all_val = self.build_augmentor(self.support_features_val, self.n_way_val, self.n_shot_val, self.n_aug_val, reuse=True) ### shape: [self.n_way*(self.n_aug-self.n_shot), self.fc_dim]
        self.hallucinated_features_val = tf.reshape(self.hal_feat_val, shape=[self.n_way_val, self.n_aug_val-self.n_shot_val, -1]) ### shape: [self.n_way, self.n_aug-self.n_shot, self.fc_dim]
        self.support_class_code_val = self.proto_encoder(self.support_feat_flat_val, reuse=True) #### shape: [self.n_way*self.n_shot, self.fc_dim]
        self.query_class_code_val = self.proto_encoder(self.query_features_val, reuse=True) #### shape: [self.n_query_all, self.fc_dim]
        self.hal_class_code_val = self.proto_encoder(self.hal_feat_val, reuse=True) #### shape: [self.n_way*(self.n_aug-self.n_shot), self.fc_dim]
        ### prototypical network data flow using the augmented support set
        self.support_encode_val = tf.reshape(self.support_class_code_val, shape=[self.n_way_val, self.n_shot_val, -1]) ### shape: [self.n_way, self.n_shot, self.fc_dim]
        self.hal_encode_val = tf.reshape(self.hal_class_code_val, shape=[self.n_way_val, self.n_aug_val-self.n_shot_val, -1]) ### shape: [self.n_way, self.n_aug-self.n_shot, self.fc_dim]
        self.support_aug_encode_val = tf.concat((self.support_encode_val, self.hal_encode_val), axis=1) ### shape: [self.n_way, self.n_aug, self.fc_dim]
        self.support_aug_prototypes_val = tf.reduce_mean(self.support_aug_encode_val, axis=1) ### shape: [self.n_way, self.fc_dim]
        self.query_tile_val = tf.reshape(tf.tile(self.query_class_code_val, multiples=[1, self.n_way_val]), [self.n_query_all_val, self.n_way_val, -1]) #### shape: [self.n_query_all, self.n_way, self.fc_dim]
        self.logits_pro_aug_val = -tf.norm(self.support_aug_prototypes_val - self.query_tile_val, ord='euclidean', axis=2) ### shape: [self.n_query_all, self.n_way]
        self.loss_pro_aug_val = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.query_labels_vec_val,
                                                                                       logits=self.logits_pro_aug_val,
                                                                                       name='loss_pro_aug_val'))
        self.acc_pro_aug_val = tf.nn.in_top_k(self.logits_pro_aug_val, self.query_labels_val, k=1)
        ...
        for epoch in range(1, (num_epoch+1)):
            ...
            if not self.val_path is None:
                for ite in tqdm.tqdm(range(1, (n_ite_per_epoch+1))):
                    ...
                    support_labels = selected_lbs
                    support_classes = np.array([self.val_class_list_raw[selected_indexes[i][0]] for i in range(self.n_way_val)])
                    loss, acc, x_tilde_i = self.sess.run([self.loss_pro_aug_val, self.acc_pro_aug_val, self.hal_feat_val],
                                                         feed_dict={self.support_features_val: support_features,
                                                                    self.query_features_val: query_features,
                                                                    self.query_labels_val: query_labels})
                    loss_ite_val.append(loss)
                    ...
                    if epoch % n_ep_per_visualization == 0 and ite == n_ite_per_epoch:
                        m_support_considered = 5
                        ###### for each class, just print the first xi and x_tilde_i
                        fnames_x_i = [self.val_fname_list[selected_indexes[lb_idx][0]] for lb_idx in range(m_support_considered)]
                        fnames_x_tilde_i = [self.val_fname_list[(np.sum(np.abs(self.val_feat_list - x_tilde_i[lb_idx*(self.n_aug_val-self.n_shot_val)]), axis=1)).argmin()] for lb_idx in range(m_support_considered)]
                        # for idx_way in range(m_support_considered):
                        #     print('============================================================')
                        #     print(fnames_x_i[idx_way])
                        #     print(fnames_x_tilde_i[idx_way])
                        ...
                        for idx_way in range(m_support_considered):
                            ...
                            img_array[idx_way+m_support_considered,:] = img
                        fig = plot(img_array, 2, m_support_considered, x_dim=x_dim)
                        plt.savefig(os.path.join(self.result_path, self.model_name, 'samples_val_ep%03d.png' % epoch), bbox_inches='tight')
            loss_train.append(np.mean(loss_ite_train))
            acc_train.append(np.mean(acc_ite_train))
...
class HAL_PN_AFHN(HAL_PN_GAN):
    ...
    def build_model(self):
        ...
        self.query_labels_val = tf.placeholder(tf.int32, shape=[self.n_query_all_val], name='query_labels_val')
        ### basic operation
        self.support_feat_flat_val = tf.reshape(self.support_features_val, shape=[-1, self.fc_dim]) ### shape: [self.n_way_val*self.n_shot_val, self.fc_dim]
        self.query_labels_vec_val = tf.one_hot(self.query_labels_val, self.n_way_val)
        ### hallucination flow
        self.support_ave_val = tf.reduce_mean(self.support_features_val, axis=1, keep_dims=True) ### shape: [self.n_way_val, 1, self.fc_dim]
        self.hal_feat_val, self.z_all_val = self.build_augmentor(self.support_ave_val, self.n_way_val, self.n_shot_val, self.n_aug_val, reuse=True) ### shape: [self.n_way_val*(self.n_aug_val-self.n_shot_val), self.fc_dim]
        self.support_class_code_val = self.proto_encoder(self.support_feat_flat_val, reuse=True) #### shape: [self.n_way_val*self.n_shot_val, self.fc_dim]
        self.query_class_code_val = self.proto_encoder(self.query_features_val, reuse=True) #### shape: [self.n_query_all_val, self.fc_dim]
        self.hal_class_code_val = self.proto_encoder(self.hal_feat_val, reuse=True) #### shape: [self.n_way_val*(self.n_aug_val-self.n_shot_val), self.fc_dim]
        ### prototypical network data flow using the augmented support set
        self.support_encode_val = tf.reshape(self.support_class_code_val, shape=[self.n_way_val, self.n_shot_val, -1]) ### shape: [self.n_way_val, self.n_shot_val, self.fc_dim]
        self.hal_encode_val = tf.reshape(self.hal_class_code_val, shape=[self.n_way_val, self.n_aug_val-self.n_shot_val, -1]) ### shape: [self.n_way_val, self.n_aug_val-self.n_shot_val, self.fc_dim]
        self.support_aug_encode_val = tf.concat((self.support_encode_val, self.hal_encode_val), axis=1) ### shape: [self.n_way_val, self.n_aug_val, self.fc_dim]
        self.support_aug_prototypes_val = tf.reduce_mean(self.support_aug_encode_val, axis=1) ### shape: [self.n_way_val, self.fc_dim]
        self.query_tile_val = tf.reshape(tf.tile(self.query_class_code_val, multiples=[1, self.n_way_val]), [self.n_query_all_val, self.n_way_val, -1]) #### shape: [self.n_query_all_val, self.n_way_val, self.fc_dim]
        self.logits_pro_aug_val = -tf.norm(self.support_aug_prototypes_val - self.query_tile_val, ord='euclidean', axis=2) ### shape: [self.n_query_all_val, self.n_way_val]
        self.loss_pro_aug_val = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.query_labels_vec_val,
                                                                                       logits=self.logits_pro_aug_val,
                                                                                       name='loss_pro_aug_val'))
        self.acc_pro_aug_val = tf.nn.in_top_k(self.logits_pro_aug_val, self.query_labels_val, k=1)
...
class HAL_PN_DFHN(object):
    ...
    def build_model(self):
        ...
        self.support_features_val = tf.placeholder(tf.float32, shape=[self.n_way_val, self.n_shot_val, self.fc_dim], name='support_features_val')
        self.query_features_val = tf.placeholder(tf.float32, shape=[self.n_query_all_val, self.fc_dim], name='query_features_val')
        self.query_labels_val = tf.placeholder(tf.int32, shape=[self.n_query_all_val], name='query_labels')
        self.pose_ref_features_val = tf.placeholder(tf.float32, shape=[self.n_way_val, self.n_aug_val-self.n_shot_val, self.fc_dim], name='pose_ref_features_val')
        ### basic operation
        self.support_feat_flat_val = tf.reshape(self.support_features_val, shape=[-1, self.fc_dim]) ### shape: [self.n_way_val*self.n_shot_val, self.fc_dim]
        self.support_feat_ave_val = tf.reduce_mean(self.support_features_val, axis=1, keep_dims=True) ### shape: [self.n_way_val, 1, self.fc_dim]
        self.pose_ref_feat_flat_val = tf.reshape(self.pose_ref_features_val, shape=[-1, self.fc_dim]) ### shape: [self.n_way_val*(self.n_aug_val-self.n_shot_val), self.fc_dim]
        self.pose_ref_feat_ave_val = tf.reduce_mean(self.pose_ref_features_val, axis=1, keep_dims=True) ### shape: [self.n_way_val, 1, self.fc_dim_val]
        self.query_labels_vec_val = tf.one_hot(self.query_labels_val, self.n_way_val)
        ### hallucination flow
        self.support_class_code_val = self.proto_encoder(self.support_feat_flat_val, reuse=True) #### shape: [self.n_way_val*self.n_shot_val, self.fc_dim]
        self.query_class_code_val = self.proto_encoder(self.query_features_val, reuse=True) #### shape: [self.n_query_all_val, self.fc_dim]
        self.pose_ref_class_code_val = self.proto_encoder(self.pose_ref_feat_flat_val, reuse=True) #### shape: [self.n_way_val*(self.n_aug_val-self.n_shot_val), self.fc_dim]
        self.support_encode_val = tf.reshape(self.support_class_code_val, shape=[self.n_way_val, self.n_shot_val, -1]) ### shape: [self.n_way_val, self.n_shot_val, self.fc_dim]
        self.support_feat_ave_tile_val = tf.tile(self.support_feat_ave_val, multiples=[1, self.n_aug_val-self.n_shot_val, 1]) ### shape: [self.n_way_val, self.n_aug_val-self.n_shot_val, self.fc_dim]
        self.support_feat_ave_tile_flat_val = tf.reshape(self.support_feat_ave_tile_val, shape=[-1, self.fc_dim]) ### shape: [self.n_way_val*(self.n_aug_val-self.n_shot_val), self.fc_dim]
        self.transformed_pose_ref_val, self.pose_code_val = self.transformer(self.pose_ref_feat_flat_val, self.support_feat_ave_tile_flat_val, reuse=True) ### shape=[self.n_way_val*(self.n_aug_val-self.n_shot_val), self.fc_dim]
        self.hallucinated_features_val = tf.reshape(self.transformed_pose_ref_val, shape=[self.n_way_val, self.n_aug_val-self.n_shot_val, -1]) ### shape: [self.n_way_val, self.n_aug_val-self.n_shot_val, self.fc_dim]
        ### prototypical network data flow using the augmented support set
        self.hal_class_code_val = self.proto_encoder(self.transformed_pose_ref_val, reuse=True) #### shape: [self.n_way_val*(self.n_aug_val-self.n_shot_val), self.fc_dim]
        self.hal_encode_val = tf.reshape(self.hal_class_code_val, shape=[self.n_way_val, self.n_aug_val-self.n_shot_val, -1]) ### shape: [self.n_way_val, self.n_aug_val-self.n_shot_val, self.fc_dim]
        self.support_aug_encode_val = tf.concat((self.support_encode_val, self.hal_encode_val), axis=1) ### shape: [self.n_way_val, self.n_aug_val, self.fc_dim]
        self.support_aug_prototypes_val = tf.reduce_mean(self.support_aug_encode_val, axis=1) ### shape: [self.n_way_val, self.fc_dim]
        self.query_tile_val = tf.reshape(tf.tile(self.query_class_code_val, multiples=[1, self.n_way_val]), [self.n_query_all_val, self.n_way_val, -1]) #### shape: [self.n_query_all_val, self.n_way_val, self.fc_dim]
        ...
        for epoch in range(1, (num_epoch+1)):
            ...
            if not self.val_path is None:
                for ite in tqdm.tqdm(range(1, (n_ite_per_epoch+1))):
                    ...
                    skip_this_episode = False
                    selected_lbs = np.random.choice(list(self.all_val_labels), self.n_way_val, replace=False)
                    selected_lbs_pose = np.random.choice(list(self.all_train_labels), self.n_way_val, replace=False)
                    try:
                        selected_indexes = [list(np.random.choice(self.candidate_indexes_each_lb_val[selected_lbs[lb_idx]], self.n_shot_val+self.n_query_all_val//self.n_way_val, replace=False)) \
                                            for lb_idx in range(self.n_way_val)]
                        selected_indexes_pose = [list(np.random.choice(self.candidate_indexes_each_lb_gallery[selected_lbs_pose[lb_idx]], self.n_aug_val-self.n_shot_val, replace=False)) \
                                                 for lb_idx in range(self.n_way_val)]
                    except:
                        print('[Validation] Skip this episode since there are not enough samples for some label')
                        skip_this_episode = True
                    ...
                    query_features = np.concatenate([self.val_feat_list[selected_indexes[lb_idx][self.n_shot_val:]] for lb_idx in range(self.n_way_val)])
                    query_labels = np.concatenate([np.repeat(lb_idx, self.n_query_all_val//self.n_way_val) for lb_idx in range(self.n_way_val)])
                    pose_ref_features = np.concatenate([self.train_feat_list_gallery[selected_indexes_pose[lb_idx]] for lb_idx in range(self.n_way_val)])
                    pose_ref_features = np.reshape(pose_ref_features, (self.n_way_val, self.n_aug_val-self.n_shot_val, self.fc_dim))
                    loss, acc = self.sess.run([self.loss_pro_aug_val, self.acc_pro_aug_val],
                                              feed_dict={self.support_features_val: support_features,
                                                         self.query_features_val: query_features,
                                                         self.query_labels_val: query_labels,
                                                         self.pose_ref_features_val: pose_ref_features})
                    loss_ite_val.append(loss)
                    acc_ite_val.append(np.mean(acc))
            loss_train.append(np.mean(loss_ite_train))
            acc_train.append(np.mean(acc_ite_train))
