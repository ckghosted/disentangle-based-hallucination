...
with open(sys.argv[1], 'r') as fhand:
    ...
    print('lr,n_shot,n_aug,novel_only_mean,novel_only_std,novel_mean,novel_std,all_mean,all_std')
        ...
        if re.search('^WARNING', line):
            if re.search('_0" already exists!', line):
                ...
                if not len(acc_list) == 0:
                    print('%s,%s,%s,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f' % \
                        (lr, n_shot, n_aug, np.mean(acc_novel_only_list)*100, np.std(acc_novel_only_list)*100, np.mean(acc_novel_list)*100, np.std(acc_novel_list)*100, np.mean(acc_list)*100, np.std(acc_list)*100))
                    acc_list = []
                    acc_novel_only_list = []
                    acc_novel_list = []
