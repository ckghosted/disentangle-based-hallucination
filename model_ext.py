...
class BasicCls(object):
    ...
    def build_model(self):
        ...
        dataset = dataset.map(self._parse_function, num_parallel_calls=4).shuffle(len(self.train_image_list)).prefetch(self.bsize).repeat().batch(self.bsize)
        iterator = dataset.make_one_shot_iterator()
        self.input_images, self.labels = iterator.get_next()
