...
import numpy as np
from model_episodic import HAL_PN_only, HAL_PN_GAN, HAL_PN_GAN2, HAL_PN_AFHN, HAL_PN_DFHN
import os, re, glob
...
def main():
    ...
    parser.add_argument('--n_way', default=5, type=int, help='Number of classes in the support set')
    parser.add_argument('--n_shot', default=1, type=int, help='Number of samples per class in the support set')
    parser.add_argument('--n_aug', default=10, type=int, help='Number of samples per class in the augmented support set')
    parser.add_argument('--n_query_all', default=75, type=int, help='Number of samples in the query set')
    parser.add_argument('--n_intra', default=0, type=int, help='Number of intra-class samples for each class in the support set')
    parser.add_argument('--n_shot_val', default=1, type=int, help='Number of samples per class in the support set for validation')
    parser.add_argument('--n_aug_val', default=10, type=int, help='Number of samples per class in the augmented support set for validation')
    ...
    parser.add_argument('--DFHN', action='store_true', help='Use pose-ref-based hallucinator if present')
...
def train(args):
    ...
    if args.exp_tag == 'cv':
        ...
        train_pickled_fname = 'base_feat'
        val_pickled_fname = 'val_feat'
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        if args.GAN:
            ...
            net = HAL_PN_GAN(sess,
                             ...
                             n_shot_val=args.n_shot_val,
                             n_aug_val=args.n_aug_val,
                             fc_dim=args.fc_dim,
                             z_dim=args.z_dim,
                             z_std=args.z_std,
...
def inference(args):
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        net.inference(hal_from=hal_from,
                      n_ite_per_epoch=args.n_ite_per_epoch)
